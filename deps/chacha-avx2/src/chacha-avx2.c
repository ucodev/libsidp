 ...
 * Public Domain
 *
 * Changes by Pedro A. Hortas (20/05/2013):
 *  - Changed function names. Added the prefix chacha_avx2_*() to avoid conflicts with other NaCl library functions.
 *  - Forced MANUAL_SCHEDULING by default through Makefile CFLAGS, since clang is the prefered compiler for SIDP project.
 *
 */
...
{
  memset(out, 0, outlen);
  return chacha_avx2_crypto_stream_xor(out, out, outlen, n, k);
}
